...
const TOOLS = [
    ...
    {
        name: 'arrange',
        description: 'Lay the whole document out in layers, mermaid-style: sources left, few crossings, even spacing. Groups are laid out as whole blocks, so their frames never overlap. Run it after building or editing, and after grouping - never place a group by hand.',
        inputSchema: object({}),
        run: () => ask('/arrange', {}),
    },
    {
    ...
    {
        name: 'launch',
        description: "Start Rhino with Grasshopper and wait for the link to answer. Use when there is no session. With fresh:true it starts another Rhino even though one is already running and works with that one - which is how two agents each get a canvas of their own instead of editing the same one.",
        inputSchema: object({ fresh: flag('Start another Rhino and use it, even if a session exists.') }),
        run: args => launch(args.fresh === true),
    },
    {
        name: 'sessions',
        description: "Every live Grasshopper session on this machine - port and Rhino process id - and which one these tools are talking to. Several agents can each work on their own canvas; use it to check you are on the one you mean.",
        inputSchema: object({}),
        run: async () => {
            const live = await sessions();
            ...
            return { using: port, sessions: live, pinned: process.env.PHENOME_GH_PORT ?? null };
        },
    },
    {
        name: 'screenshot',
...
function instructions() {
    ...
    return [
        ...
        '9. Never position anything by hand: `arrange` lays groups out as blocks.',
        '10. Finish with `signature` once, `arrange`, then `review` - fix every finding marked blocking,',
        '    treat polish as optional - then `save`.',
        '',
    ...
    switch (method) {
        case 'initialize':
            reply(id, {
                ...
                capabilities: { tools: {} },
                serverInfo: { name: 'grasshopper', version: '0.19.0' },
